...
class KintoneForm {
	...
	public function kintone_form_tag_generator( $contact_form, $args = '' ) {
					...
					foreach ( $appdata['formdata']['properties'] as $form_data ) {
						...
						if ( isset( $form_data['code'] ) ) {
							...
							$code = wp_strip_all_tags(
								$this->create_sample_shortcode( $form_data, $select_option, $original_cf7tag_name, '' )
							);
							if ( $code ) {
		...
		<div class="insert-box">
			...
			<div class="submitbox">
				<input type="button" class="button button-primary kintone-form-insert-tag" value="<?php echo esc_attr(
					__( 'Insert Tag', 'kintone-form' )
				); ?>"/>
			</div>
	...
	public function register_assets() {
		...
		wp_register_script(
			'my_loadmore',
			plugin_dir_url( __FILE__ ) . 'asset/js/myloadmore.js',
			array( 'jquery' ),
			'',
			true
		);
	...
	public function form_data_to_kintone_setting() {
		...
		if ( ! empty( $_POST ) && check_admin_referer( $this->nonce ) ) {
			...
			if ( isset( $_POST['kintone_to_wp_license_key_attachment'] ) && ! empty( $_POST['kintone_to_wp_license_key_attachment'] ) ) {
				...
				$kintone_to_wp_license_key_attachment = sanitize_text_field(
					$_POST['kintone_to_wp_license_key_attachment']
				);

				update_option(
					'_kintone_to_wp_license_key_attachment',
					base64_encode( $kintone_to_wp_license_key_attachment )
				);
...
//				$license_result_attachment = $this->gumroad_verify_license($kintone_to_wp_license_key_attachment, GUMROAD_KINTONE_FORM_ATTACHMENTS_SLUG);
			...
			if ( isset( $_POST['kintone_to_wp_license_key_multiple_kintone_app'] ) && ! empty( $_POST['kintone_to_wp_license_key_multiple_kintone_app'] ) ) {
				...
				$kintone_to_wp_license_key_multiple_kintone_app = sanitize_text_field(
					$_POST['kintone_to_wp_license_key_multiple_kintone_app']
				);
				update_option(
					'_kintone_to_wp_license_key_multiple_kintone_app',
					base64_encode( $kintone_to_wp_license_key_multiple_kintone_app )
				);
...
//				$license_result_multiple_kintone_app = $this->gumroad_verify_license($kintone_to_wp_license_key_multiple_kintone_app, GUMROAD_KINTONE_FORM_MULTI_KINTONE_APP_SLUG);
		...
		$kintone_to_wp_license_key_attachment           = base64_decode(
			get_option( '_kintone_to_wp_license_key_attachment' )
		);
		$kintone_to_wp_license_key_multiple_kintone_app = base64_decode(
			get_option( '_kintone_to_wp_license_key_multiple_kintone_app' )
		);
					...
					<form method="post" action="">
						...
						<table class="form-table">
							<tr valign="top">
								...
								<td>
									<input name="kintone_to_wp_license_key_attachment" type="text" id="kintone_to_wp_license_key_attachment" value="<?php echo( $kintone_to_wp_license_key_attachment == "" ? "" : esc_textarea(
										$kintone_to_wp_license_key_attachment
									) ); ?>" class="regular-text"/>
									<?php if ( is_wp_error( $license_result_attachment ) ): ?>
										<br>
										<span style="color:red;"><?php echo $license_result_attachment->get_error_message(
											); ?></span>
									<?php endif; ?>
								</td>
							</tr>
							<tr valign="top">
								<th scope="row"><label for="add_text">License key : Add-on Multiple kintone app</label>
								</th>
								<td>
									<input name="kintone_to_wp_license_key_multiple_kintone_app" type="text" id="kintone_to_wp_license_key_multiple_kintone_app" value="<?php echo( $kintone_to_wp_license_key_multiple_kintone_app == "" ? "" : esc_textarea(
										$kintone_to_wp_license_key_multiple_kintone_app
									) ); ?>" class="regular-text"/>
									<?php if ( is_wp_error( $license_result_multiple_kintone_app ) ): ?>
										<br>
										<span style="color:red;"><?php echo $license_result_multiple_kintone_app->get_error_message(
											); ?></span>
									<?php endif; ?>
								</td>
							</tr>
		...
		<fieldset>
			...
			<table>
				<tr>
					...
					<td>
						<input type="text" id="kintone-form-domain" placeholder="xxxx.cybozu.com" name="kintone_setting_data[domain]" class="" size="70" value="<?php echo esc_attr(
							$domain
						); ?>"/></td>
				</tr>
				<tr>
					...
					<td>
						<input type="text" id="email-address-to-send-kintone-registration-error" name="kintone_setting_data[email_address_to_send_kintone_registration_error]" class="" size="70" value="<?php echo esc_attr(
				...
				<tr>
					...
					<td>
						ID：
						<input type="text" id="kintone-basic-authentication-id" name="kintone_setting_data[kintone_basic_authentication_id]" class="" size="30" value="<?php echo esc_attr(
							$kintone_basic_authentication_id
						); ?>"/> / Password：
						<input type="password" id="kintone-basic-authentication-password" name="kintone_setting_data[kintone_basic_authentication_password]" class="" size="30" value="<?php echo esc_attr(
							$kintone_basic_authentication_password
						); ?>"/>
					</td>
				</tr>
			...
			<div class="repeat">
				<div id="kintone_form_setting" class="wrapper" style="border-collapse: collapse;">
								...
								<table class="row" style="margin-bottom: 30px; border-top: 6px solid #ccc; width: 100%;">
									<tr>
										<td valign="top" style="padding: 10px 0px;">
											APP
											ID:<input type="text" id="kintone-form-appid" name="kintone_setting_data[app_datas][<?php echo $i; ?>][appid]" class="small-text" size="70" value="<?php echo esc_attr(
												$app_data['appid']
											); ?>"/>
											Api
											Token:<input type="text" id="kintone-form-token" name="kintone_setting_data[app_datas][<?php echo $i; ?>][token]" class="regular-text" size="70" value="<?php echo esc_attr(
												$app_data['token']
											); ?>"/>
												...
												<?php if ( isset( $app_data['formdata']['properties'] ) ): ?>
													<?php foreach ( $app_data['formdata']['properties'] as $form_data ): ?>
														...
														if ( isset( $form_data['code'] ) ):
															...
															<tr>
																<td style="padding: 5px 10px; border-bottom: 1px solid #e2e2e2;"><?php echo esc_html(
					...
					<tr>
						<td colspan="2">
							<span class="add button">追加</span> ← <a href="<?php echo admin_url(
								'admin.php?page=form-data-to-kintone-setting'
							); ?>" title="">Add-Ons</a>
						</td>
					</tr>
	...
	public static function decode( $encoded ) {
		return preg_match( '/^[a-f0-9]{32}$/', $encoded ) ? $encoded : str_replace(
			array( md5( AUTH_SALT ), md5( md5( AUTH_SALT ) ) ),
			'',
